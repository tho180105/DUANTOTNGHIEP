...
app.controller("cart-ctrl", function ($rootScope,$http,$scope,$timeout) {
    ...
    $scope.decreaseQuantity= function(element){
        ...
        if(oldValueNumber==1){
            ...
            $scope.update(element.currentTarget.getAttribute("detailCartId"),oldValueNumber-1)
        }
    ...
    var checkExist=$rootScope.detailCarts.findIndex(item => item.productRepository.productrepositoryid == productRepositoryId);
        if(checkExist==-1){
            $http.post("/rest/cart/"+ productRepositoryId).then(resp => {
            ...
            $http.put("/rest/cart", $rootScope.detailCarts[checkExist]).then(resp => {
            }).catch(error => {
                alert("Thay đổi số lượng lỗi")
                console.log(error)
            })
    ...
    $scope.update=function(detailCartId,quantity){
        var detailCart=$rootScope.detailCarts.find(item => item.detailcartid == detailCartId);
        detailCart.quantity=quantity
        $http.put("/rest/cart", detailCart).then(resp => {
    ...
    $scope.delete =function(detailCartId){
        var index = $rootScope.detailCarts.findIndex(item => item.detailcartid == detailCartId);
        $rootScope.detailCarts.splice(index, 1);
        $http.delete("/rest/cart/"+detailCartId).then(resp => {
        }).catch(error => {
            alert("Xóa  lỗi")
            console.log(error)
        })
